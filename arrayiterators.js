...
const oldestYoungest = inventors.sort((a,b) => a.year - b.year)
...
const allYearsAlive = inventors.reduce(function(acc, yearsAlive){
...
const nameReverse = people.map((name) =>
  name.split(',').reverse())
...
let tally = data.reduce((acc, tal) => {
  acc[tal] = acc[tal] ? acc[tal] + 1 : 1;
  return acc;
}, {}); 
...
let older19 = devs.some((dev) => (2020 - dev.year) >= 19)
...
let everyone19 = devs.every((dev) => (2020 - dev.year) >= 19)
...
let good = comments.find((comment) => comment.id === 823423)
...
let ramen = comments.findIndex((comment) => comment.id === 123523)
